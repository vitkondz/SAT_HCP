...
class Unary(SuccessorMethod):
    ...
    def vertex_start(self, cnf, n):
        for i in range(2, n+1):
            cnf.append([-self.getH(1, i), self.getU(i, 2)])

    # Hi1 -> Uin - constraints (4')
    def vertex_end(self, cnf, n):
        for i in range(2, n+1):
            cnf.append([-self.getH(i, 1), self.getU(i, n)])
            
    # Hij ^ Uip -> Uj(p+1) - constraints (5')
    def vertex_positions(self, cnf, n):
        for i in range(2, n+1):
    ...
    def build_clauses(self, cnf, graph):
        ...
        self.vertex_start(cnf, n)
        self.vertex_end(cnf, n)
        self.vertex_positions(cnf, n)
